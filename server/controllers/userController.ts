...
export const recordDiagnosis: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    // Create diagnosis with encrypted sensitive data only
    const diagnosis = await prisma.diagnosis.create({
      data: {
        userId: req.user.id,
        symptomsEncrypted: encryptField(symptoms), // Encrypted symptoms
        resultsEncrypted: encryptField(results), // Encrypted results (now array of all results)
        diseaseId: primaryDiseaseId // Primary/most likely disease
      },
...
export const getDiagnosisStats: RequestHandler = async (req, res) => {
  try {
    ...
    const recentActivities = recentDiagnoses.map(diagnosis => ({
      id: diagnosis.id,
      type: 'diagnosis',
      description: diagnosis.disease ? `Diagnosed with ${diagnosis.disease.name}` : 'Health check completed',
